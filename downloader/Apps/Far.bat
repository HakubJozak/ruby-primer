...
rem ===				Set Variables				===
set AppName=Far
echo https://www.farmanager.com/download.php?l=en > TMP\%AppName%.get-url
set REGEX_get-string=(Far)\S+(x64).+(.msi)
set REGEX_get-version=\d{2}b\d{4}
...
for /f "tokens=*" %%b in (TMP\%AppName%.ver-str) do (set FileName=%%b)
 
echo https://www.farmanager.com/files/%FileName% > TMP\%AppName%.dow-url
rem ===============================================
