...
rem ===				Set Variables				===
set AppName=Firefox
echo http://download.mozilla.org > TMP\%AppName%.get-url
set REGEX_get-string=^(?:data-latest-firefox^=.^)^(\d{2}.\d{1}.\d{1}^).
...
echo http://download.mozilla.org/?product=firefox-%Version%^&os=win^&lang=cs > TMP\%AppName%.dow-url
rem ===============================================
