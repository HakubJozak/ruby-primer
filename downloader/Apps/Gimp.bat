...
rem ===				Set Variables				===
set AppName=Gimp
echo https://www.gimp.org/downloads/ > TMP\%AppName%.get-url
set REGEX_get-string=(?:The current stable release of GIMP is ^<b^>)(\d{1,2}.\d{1,2}.\d{1,2})
set REGEX_get-version=\d{1,2}.\d{1,2}.\d{1,2}
...
grep -oP "(download.gimp.org/mirror/pub/gimp/v\d.\d).+(.exe)" TMP\%AppName%.get-ver 1> TMP\%AppName%.url-str
grep -oP "(download).+.(.exe)" TMP\%AppName%.url-str > TMP\%AppName%.dow-url
rem ===============================================
