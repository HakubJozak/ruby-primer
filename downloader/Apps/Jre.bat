...
rem ===				Set Variables				===
set AppName=Jre
echo https://java.com/en/download/manual.jsp > TMP\%AppName%.get-url
set REGEX_get-string=(Recommended Version)\s\d\s(Update)\s\d{3}
set REGEX_get-version=\d\s(Update)\s\d{3}
...
set Version=%Version: Update =u%


grep -oP -m 1 "(?:<a title=\"Download Java software for Windows Offline\" href=\")(\S*)(?:\">)" TMP\%AppName%.get-ver 1> TMP\%AppName%.url-str
grep -oP -m 1 "(http://).+(\d)" TMP\%AppName%.url-str > TMP\%AppName%.dow-url
rem ===============================================
