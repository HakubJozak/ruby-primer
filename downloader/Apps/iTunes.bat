...
rem ===				Set Variables				===
set AppName=iTunes
echo https://swdlp.apple.com/iframes/82/en_us/82_en_us.html > TMP\%AppName%.get-url
set REGEX_get-string=^(?^<=iTunes ^)[0-9.]{2,}^(?= for Windows \^(64-bit\^)^)
set REGEX_get-version=\d{2}\.\d{1}.\d{1}
...
grep -oP "(system_OS_Hide_Show:6:winxpvistax64:64bit:OSdetect:radio:binaryUrl' value=')\S*'" TMP\%AppName%.get-ver 1> TMP\%AppName%.url-str
grep -oP "(https://).+.(.exe)" TMP\%AppName%.url-str > TMP\%AppName%.dow-url
rem ===============================================
